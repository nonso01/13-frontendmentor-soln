...
function App() {
  ...
  root.onanimationiteration = () => {
    loadingCount >= HUNDRED
      ? setLoadingCount(HUNDRED)
  ...
  function handleSubmit(e) {
    ...
    if (!wrongEmailFormat && isEmail) setSubmit((n) => !n);
  ...
  if (!loaded) return <Loader count={loadingCount} />;
  else if (loaded && !submit)
    return (
      <SignUp
  ...
  else if (loaded && submit && !dismiss)
    return <Success email={email} handleDismiss={handleDismiss} />;
  else return <AboutMe />;
}
export default App;
